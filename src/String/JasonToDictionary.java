...
public class JasonToDictionary {
    ...
    /**      * Json to Dict, recursion      * @param json json to be converted      * @return Dict      */
    public Dict jsonToDict(String json) {
        return helper(json, 1).dict;
    }

    /**      * ResultType contains two filed: returned dict and end index      */
    class ResultType {
        int index;
        Dict dict;
    ...
    private ResultType helper(String json, int index) {
        ...
        while (index < json.length()){
            ...
            if (json.charAt(index) == '{') {
                ...
                left = index;
                map.put(key, r.dict);                 //continue;
            }

            if (json.charAt(index) == '}') {                 //two cases before each '}' : string(value) or '}'
    ...
    public static void main(String[] args) {
        JasonToDictionary s = new JasonToDictionary();
        s.test();
    }
}

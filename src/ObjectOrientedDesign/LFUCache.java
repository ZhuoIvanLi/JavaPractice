 ...
 *
 * Solution: similar to LRU
 */
public class LFUCache {
    private Node head = new Node(0, 0, 0), tail = new Node(0, 0, 0);
    ...
    private void countPlus(Node n) {
        n.count++;

        // Reorder its place
        Node nodeNext = n.next;
        if (nodeNext != tail && nodeNext.count <= n.count){
            // Notice: When nodeNext.next count greater or equal to current node count, we stop loop. Add current after nodeNext
            while (nodeNext != tail && nodeNext.next.count <= n.count) {
            ...
            // Add it after target node
            addAfter(n, nodeNext);
        }
    }

    private void addAfter(Node n, Node target) {
        n.prev = target;
        n.next = target.next;
        target.next = n;
        n.next.prev = n;
